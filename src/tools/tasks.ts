...
export function addTask(userId: string, title: string, dueAt?: string): Task {
  const stmt = db.prepare(
    "INSERT INTO tasks (user_id, title, due_at) VALUES (?, ?, ?)"
  );
  const info = stmt.run(userId, title, dueAt ?? null);
  return db
    .prepare("SELECT * FROM tasks WHERE id = ?")
    .get(info.lastInsertRowid) as Task;
}

export function listTasks(userId: string, includeDone = false): Task[] {
  const query = includeDone
    ? "SELECT * FROM tasks WHERE user_id = ? ORDER BY done ASC, due_at IS NULL, due_at ASC, id ASC"
    : "SELECT * FROM tasks WHERE user_id = ? AND done = 0 ORDER BY due_at IS NULL, due_at ASC, id ASC";
